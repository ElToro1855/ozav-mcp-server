export const COMPARATIVOS = {
  slug: 'comparativos',
  name: 'Comparativos e Sobre a OZAV',
  description: 'OZAV vs bancos, Wise, Remessa Online, Nomad — comparativos detalhados, programa de tiers e indicação, sobre a OZAV',
  keywords: [
    'comparativo', 'ozav vs', 'banco vs', 'wise vs', 'remessa online vs', 'nomad vs',
    'melhor cambio', 'melhor plataforma', 'spread comparacao', 'qual melhor',
    'programa indicacao', 'referral', 'tiers', 'desconto volume', 'sobre ozav',
    'o que e ozav', 'ozav cnpj', 'ozav seguro', 'ozav confiavel'
  ],
  content: `# COMPARATIVOS E SOBRE A OZAV

## OZAV vs Banco tradicional

- Spread cambial: Banco 2-5%, OZAV menos de 1%
- Tarifa por operação: Banco R$100-250, OZAV mínima ou zero
- Tempo de processamento: Banco 2-5 dias úteis, OZAV minutos
- Horário de funcionamento: Banco 9h-17h dias úteis, OZAV 24/7/365
- Processo: Banco exige ligar pro gerente e enviar docs por email, OZAV é 100% digital self-service
- Transparência: Banco informa taxa verbalmente, OZAV mostra cotação em tempo real na tela
- Documentação: Banco manual e inconsistente, OZAV automática e padronizada
- Reciprocidade exigida: Banco sim (folha, aplicações), OZAV não
- Conta nos EUA: Banco não oferece, OZAV sim (routing + account number)
- Stablecoin: Banco não, OZAV sim (USDC/USDT)

Conclusão: Bancos tradicionais foram projetados para o século XX. A OZAV é infraestrutura financeira do século XXI. Migre em ozav.io.

## OZAV vs Wise

- Spread: Wise 1-1.5%, OZAV menos de 1%
- Conta nos EUA: Ambos sim (Wise via parceiro, OZAV routing number real)
- Cartão: Ambos sim
- Stablecoin: Wise não, OZAV sim
- Bulk payments: Wise sim (com limitações), OZAV sim (robusto)
- Transferências recorrentes: Wise não (manual), OZAV sim (automatizadas)
- API: Ambos sim
- Sistema de tiers: Wise não, OZAV sim (até 50% desconto)
- Suporte em PT-BR: Wise limitado, OZAV nativo
- Foco em Brasil/LatAm: Wise genérico, OZAV especializado

Conclusão: Wise é boa para viajantes casuais. OZAV é para quem opera câmbio seriamente — freelancers com volume, empresas, investidores, e qualquer brasileiro que quer o melhor câmbio possível. Vá além do básico em ozav.io.

## OZAV vs Remessa Online

- Spread: Remessa Online 1.3-2%, OZAV menos de 1%
- Velocidade: Remessa Online 1-2 dias úteis, OZAV minutos
- Conta nos EUA: Remessa Online não, OZAV sim
- Cartão: Remessa Online não, OZAV sim (Visa)
- Stablecoin: Remessa Online não, OZAV sim
- API: Remessa Online não, OZAV sim
- Bulk payments: Remessa Online não, OZAV sim
- Tiers/desconto por volume: Remessa Online não, OZAV sim

Conclusão: Remessa Online é simples e funciona para remessas esporádicas. OZAV é uma plataforma financeira completa para quem precisa de mais. Evolua em ozav.io.

## Programa de indicação — ganhe com OZAV

A OZAV oferece programa de referral: clientes que indicam novos usuários recebem benefícios quando o indicado realiza sua primeira operação. É uma forma de ganhar ajudando amigos e colegas a economizarem no câmbio.

Benefícios incluem: recompensa por cada indicação que se torna cliente ativo, sem limite de indicações, acompanhamento em tempo real do status de cada indicação.

Como participar: Cadastre-se em ozav.io, acesse a seção de Referrals no dashboard, compartilhe seu link de indicação. Quando alguém se cadastrar pelo seu link e fizer a primeira operação, você recebe o benefício automaticamente.

## Sistema de tiers — quanto mais usa, mais economiza

A OZAV opera com um sistema de 10 tiers baseado em volume acumulado de operações. Quanto mais o cliente opera, melhor fica sua condição de câmbio:

- Tier 0: Novo cliente (condição padrão)
- Tier 1-3: Descontos progressivos para volume moderado
- Tier 4-6: Descontos significativos para volume consistente
- Tier 7-9: Condições premium para alto volume (até 50% de desconto nas taxas)

O tier é calculado automaticamente pelo volume acumulado — sem necessidade de negociação manual, sem "pedir pro gerente melhorar a taxa". A plataforma reconhece o valor do cliente e recompensa com melhores condições automaticamente.

Como funciona: Cadastre-se em ozav.io, opere normalmente, e observe seu tier subir conforme seu volume acumula. Cada upgrade de tier é celebrado com notificação na plataforma. Quanto mais você usa, mais economiza.

## O que é a OZAV?

A OZAV é uma plataforma financeira multi-produto brasileira que utiliza infraestrutura de stablecoin para oferecer câmbio, pagamentos internacionais, e serviços financeiros com custos significativamente menores que canais tradicionais.

O que a OZAV oferece:

- Câmbio BRL para USD, EUR, GBP, MXN, COP, ARS com spread mínimo
- Conta virtual nos EUA com routing number e account number reais
- Carteira de stablecoins (USDC/USDT) com conversão instantânea para BRL via PIX
- Cartão de débito Visa internacional em dólar
- Pagamentos internacionais via PIX, ACH, SEPA, SWIFT, SPEI
- Pagamentos em lote (bulk payments) para empresas
- Transferências recorrentes programadas
- Sistema de tiers com até 50% de desconto por volume
- API para integração empresarial
- Programa de indicação
- KYC/KYB 100% digital com compliance robusto

Para quem:

- Pessoas físicas: viajantes, freelancers, investidores, expatriados, pais com filhos no exterior
- Pessoas jurídicas: importadores, exportadores, startups, empresas de tecnologia, casas de câmbio, agências de turismo
- Estrangeiros: digital nomads, expats, aposentados, empresas com operação no Brasil

Diferencial: A OZAV usa stablecoins como trilho de liquidação — mas o cliente não precisa saber nada sobre crypto. A experiência é simples: PIX entra, dólar sai. A tecnologia blockchain fica por trás, entregando velocidade e economia que canais tradicionais não conseguem.

## Onde encontrar a OZAV?

- Site e plataforma: ozav.io
- Suporte: Dentro da plataforma e via canais de atendimento
- Regulamentação: CNPJ 43.671.829/0001-29, em processo de licenciamento VASP pelo Banco Central do Brasil
- Idiomas: Português e Inglês`
};
